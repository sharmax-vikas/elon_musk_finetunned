...
def _parse_turn_from_line(line, user_tag="INTERVIEWER:", assistant_tag="Elon Musk:"):
    if "Interviewer:" in line:
        return {"role": "user", "content": line.replace("Interviewer:", "").strip()}
    elif "Elon Musk:" in line:
        return {"role": "assistant", "content": line.replace("Elon Musk:", "").strip()}
    else:
...
def _construct_conversations_from_interview(
    ...
    min_assistant_turns = random.randint(assistant_min_sample_turns, assistant_max_sample_turns)
    ...
    for line in interview_lines:
        if current_assistant_turns >= min_assistant_turns and current_user_turns >= 1:
            # Only add non-empty conversations with both user and assistant turns
            if any(turn["role"] == "user" for turn in current_conversation) and any(turn["role"] == "assistant" for turn in current_conversation):
        ...
        turn = _parse_turn_from_line(line, user_tag="INTERVIEWER:", assistant_tag="Elon Musk:")
        if turn["role"] == "same":
            if len(current_conversation) > 0:
                current_conversation[-1]["content"] += " " + turn["content"]
            continue
        elif turn["role"] == "assistant":
            if len(current_conversation) > 0 and current_conversation[-1]["role"] == "assistant":
                current_conversation[-1]["content"] += " " + turn["content"]
                continue
            ...
            if len(current_conversation) > 0 and current_conversation[-1]["role"] == "user":
                current_conversation[-1]["content"] += " " + turn["content"]
                continue
            current_user_turns += 1
            current_conversation.append(turn)

    # Add the last conversation if valid
    if current_conversation and any(turn["role"] == "user" for turn in current_conversation) and any(turn["role"] == "assistant" for turn in current_conversation):
        conversations.append(current_conversation)

    # Filter out any empty conversations
    return [conv for conv in conversations if len(conv) > 0]
...
def construct_trainset(clean_transcripts_dir: str, outdir: str):
    ...
    for file_path in file_paths:
        ...
        if not conversations:
            print(f"Warning: No conversations found in {file_path}. Skipping file.")
            continue
        print(file_path)
...
construct_trainset(
    clean_transcripts_dir="data/transcripts/raw",
